...
public class PlayerManager : MonoBehaviour
{
    ...
    public enum MOVE_DIR {STOP, LEFT, RIGHT,};      // 移動方向定義
    ...
    void Update()
    {
        ...
        if (!usingButtons)
        {
            ...
            if (Input.GetKeyDown("space")) PushJumpButton();
        }
    }
	...
	{
        // プレイ中でなければ衝突判定は行わない
        if (gameManager.GetComponent<GameManager>().gameMode != GameManager.GAME_MODE.PLAY) return;
        ...
        if (col.gameObject.tag == "Goal") gameManager.GetComponent<GameManager>().GameClear();
